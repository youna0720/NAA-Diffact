...
#            outputs['next'] = output_next
    ...
    def ddim_sample(self, past_label, inputs, seed=None):
        ...
        for time, time_next in time_pairs:
            ...
            ###########################decoder start################################3
            time_emb = get_timestep_embedding(time_cond, self.transformer.time_emb_dim)
            time_emb = self.transformer.time_in[0](time_emb)        
            time_emb = swish(time_emb)
            time_emb = self.transformer.time_in[1](time_emb)
            #time_emb [1 512]  512= time_emb_dim   -> 다 음수? 근데 맞겠지
            
            fra = self.transformer.conv_in(x_m.float())    #nn.Conv1d(num_classes, num_f_maps, 1)  # [32 512 1]  ???????????
        ...
        [[ 2.5625e-02],
         ...
         [-5.0378e-02]],
            ...
            if time_emb is not None:
                fra = fra + self.transformer.time_proj(swish(time_emb))[:,:,None]      # 동일하게 1 24 1602    [32 512 1]
            ...
            x_start = self.fc_next(fra)   #[32 512 1] -> [32 2513 1]          [[[-0.6582], [-0.6588], [ 0.0841],     
            #           self.fc_next = nn.Linear(hidden_dim, self.n_class) 32 1 512 -> 32 1 2513
            '''
            [[[ 0.0118,  0.1659, -0.1253,  ...,  0.2785, -0.0968, -0.5739]],
            [[ 0.3108,  0.4942, -0.3866,  ..., -0.1170, -0.0146, -0.1195]],
            '''
            x_start = rearrange(x_start, 'b t c -> b c t')                                    #32 2513 1
            '''
            [[[-0.5237],           피드백후 [ [[ 0.1709],  [-0.0917],  [-0.0190], .. ] , 
            [-0.6955],
            [ 0.2386],
            ...,
            '''
            ...
            '''
            tensor([[[0.0002],      피드백후 [[[0.0014],[0.0011],[0.0012],...,
            [0.0002],                    작지만 가능성있어 길이가 2513이잖아
            [0.0004],
            ...,
            '''
...
#################################################################################################################3
    def ddim_func(self, past_label, inputs):
        ...
        #Features to hidden dim
        src = self.input_embed(src)              #  self.input_embed = nn.Linear(feature_dim, hidden_dim)
        src = F.relu(src)
